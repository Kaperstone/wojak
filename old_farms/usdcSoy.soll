...
contract UsdcSoy is ERC20, AccessControlEnumerable {
    ...
    event Withdraw(uint usdcAmount);
    ...
    ILending public constant LTOKEN = ILending(0xE45Ac34E528907d0A0239ab5Db507688070B20bf); // The token from the lending platform we interact with
    IERC20 public constant ITOKEN = IERC20(0x04068DA6C83AFCFA0e13ba15A6696662335D5B75); // The token we are investing with, like USDC
    ...
    uint public usdcBalance = 0;
    uint public epochIndex = 0;
    uint public usdcOverflow = 0;
    ...
    mapping(uint => uint) private usdcHistory; // Will keep a track of his share of the pool

    address ownerAddress = address(0);

    constructor() ERC20("USDC Bean", "usdcSOY") {
    ...
    function depositAdmin(uint usdcAmount) public onlyRole(CONTRACT_ROLE) {
        _deposit(usdcAmount);
    }

    function deposit(uint usdcAmount) public {
        require(!lock, "UscSoy:Distribution is going on");
        require(usdcAmount >= 10**(_decimals-2), "UscSoy:Minimum 0.01 USDC");
        _deposit(usdcAmount);
    }

    function _deposit(uint usdcAmount) private {
        require(!disabled, "UscSoy:Farm disabled,withdraw only");
        
        // We transfer his tokens to the smart contract, its now in its posession
        ITOKEN.safeTransferFrom(msg.sender, address(this), usdcAmount);
        ...
        usdcBalance += usdcAmount; // Global track
        usdcOverflow += usdcAmount;

        // Deposit to lending
        ITOKEN.approve(address(LTOKEN), usdcAmount);
        LTOKEN.mint(usdcAmount); // Put into work
        
        _mint(msg.sender, usdcAmount); // Mint 1:1 $SOY tokens with USDC

        emit Deposit(usdcAmount);
        IKeeper(keeper).distributeRewards();
    }
    
    function withdraw(uint soyAmount) public returns (uint) {
        // 7 epochs
        require(epochIndex > (epochs[msg.sender] + 7), "UscSoy:You cannot withdraw yet.");
        require(!lock, "UscSoy:Distribution is going on");
    ...
    function _withdraw(uint soyAmount) private onlyRole(FARMERS) returns (uint) {
        ...
        require(balanceOf(msg.sender) >= soyAmount, "UscSoy:Insufficient amount");
        
        // Burn his sauce
        _burn(msg.sender, soyAmount);
        // Decrease global supply
        usdcBalance -= soyAmount;
    ...
    function _withdrawRewards(uint soyAmount) private returns (uint) {
        ...
        if(cEpoch < epochIndex)
            ...
            for(uint x = cEpoch; x <= epochIndex; x++) 
                if(zwjkHistory[x] > 0 && usdcHistory[x] > 0) 
                    zwjkAmount += (zwjkHistory[x]*10**(18 - _decimals + 18)) / ((usdcHistory[x]*10**(36 - _decimals)) / soyAmount);
    ...
    function distributeRewards() public onlyRole(CONTRACT_ROLE) {
        if(usdcBalance > 0) {
            ...
            uint usdcProfit = 0;
            if(underlyingBalance > usdcBalance) usdcProfit = underlyingBalance - usdcBalance;

            if(usdcProfit > 10**(_decimals-2)) { // 0.01
                // Perform a buyback and then put into staking all the tokens
                // Extract precise amount of revenue


                // Get accrued revenue
                LTOKEN.redeemUnderlying(usdcProfit);
                Comptroller.claimComp(address(this));
                uint screamBalance = SCREAM.balanceOf(address(this));
                if(screamBalance > 1e18) { // 1.0 scream
                    uint wftmAmount = swap(address(SCREAM), WFTM, screamBalance, address(this));
                    swap(WFTM, address(ITOKEN), wftmAmount, address(this));
                }
                uint usdc = ITOKEN.balanceOf(address(this));

                // 5% to locker
                uint lockerShare = usdc / 20;
                ITOKEN.approve(address(locker), lockerShare);
                locker.depositToBank(address(ITOKEN), lockerShare);
                usdc -= lockerShare;

                // 1% to dev
                uint devShare = usdc / 100;
                ITOKEN.transfer(address(ownerAddress), devShare);
                usdc -= devShare;

                uint wjkAmount = swap(address(ITOKEN), address(wjk), usdc, address(this));
            ...
            usdcHistory[epochIndex] = usdcBalance - usdcOverflow;
            usdcOverflow = 0;
            epochIndex++;
