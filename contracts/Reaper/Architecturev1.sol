...
abstract contract Architecturev1 is ERC20, AccessControlEnumerable {
    ...
    struct Depositor {
        ...
        uint index;
        uint justDeposited;
        mapping(uint => uint) iEpochChanged;
    ...
    function _deposit(uint tokenAmount) private {
        ...
        if(balanceOf(msg.sender) == 0) {
            ...
            if(depositor[msg.sender].epoch == epochIndex) {
                // He did deposit this epoch, so we use the same 
                index = depositor[msg.sender].index;
                depositor[msg.sender].justDeposited += tokenAmount;
            }else{
        ...
        depositor[msg.sender].iEpochChanged[index] = epochIndex + 1; // Count the change in, in the next epoch
        depositor[msg.sender].myBalance[index] = balanceOf(msg.sender); // Record new balance

        depositor[msg.sender].epoch = epochIndex; // This is his sole purpose.
    ...
    function _withdraw(uint soyAmount) private onlyRole(FARMERS) returns (uint) {
        ...
        if(depositor[msg.sender].epoch == epochIndex) {
            ...
            */
            ...
            wjkAmount += withdrawRewards(msg.sender);
        ...
        depositor[msg.sender].iEpochChanged[index] = epochIndex; // Record new change on this epoch, because the tokens are immediately withdrawn
        depositor[msg.sender].myBalance[index] = balanceOf(msg.sender) - soyAmount; // Record new balance
    ...
    */
    function withdrawRewards(address _holder) public onlyRole(FARMERS) returns (uint) {  // Error: revert
    ...
    */
    function checkRewards(address _holder) public view returns (uint) {
        ...
        if(hasRole(FARMERS, _holder) && lastReceive != epochIndex) {

            uint index = 0;
            uint zwjkRewards = 0;

            // We determine where the index should start from
            // the iEpochChanged determines when there was a sudden change in balance (increase, or decrease)
            // The balance is the one that was before the last receive, becuase lastReceive isn't set on balance change
            // It is set when the user receives the reward.
            for(uint x = 0; x < depositor[_holder].index; x++) {
                /*
                    epochIndex = 8
                    iEpochChanged = [4, 5, 6, 7]
                    lastReceive = 5
                ...
                */
                if(depositor[_holder].iEpochChanged[x] >= lastReceive) {
                    index = x;
                    break;
                }
            }
            uint balance = depositor[_holder].myBalance[index];
            /*
            ...
            */
            ...
            if(epochIndex > lastReceive) { // Always excluding current epoch
                for(uint x = lastReceive; x < epochIndex; x++) {
                    if(zwjkHistory[x] > 0 && tokenHistory[x] > 0 && balance > 0)
                        zwjkRewards += (zwjkHistory[x]*10**(18 - _decimals + 18)) / ((tokenHistory[x]*10**(36 - _decimals)) / balance);
                    
                    // If spokenEpoch equals to the new change
                    // then we move to the next index and use the new balance
                    if(x == depositor[_holder].iEpochChanged[index])
                        balance = depositor[_holder].myBalance[++index];
                }
            }
    ...
    // function checkRewardForEpoch(address _holder, uint epoch) public view returns (uint) {
    //     uint index = 0;

    //     for(uint x = 0; x < depositor[_holder].index; x++) {
    //         if(depositor[_holder].iEpochChanged[x] >= epoch) {
    //             if(x == 0) index = 0;
    //             else index = x -1;
    //             break;
    //         }
    //     }
    //     uint zwjkRewards = (zwjkHistory[x]*10**(18 - _decimals + 18)) / ((tokenHistory[x]*10**(36 - _decimals)) / balance);
    //     return zwjkRewards;
    // }
    ...
    function distributeRewards() public onlyRole(CONTRACT_ROLE) {
        ...
        if(cBalance > 0) {
            ...
            if(address(ITOKEN) == address(USDC)) {
                ...
                fraction = IERC20(ITOKEN).totalSupply()*1e18 / 100000000;
            }
            if(profit > fraction) { // 0.0001
